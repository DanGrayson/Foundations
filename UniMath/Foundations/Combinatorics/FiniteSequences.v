...
Definition sequenceEquality' {X} (f g : Sequence X) (p : length f = length g) :
...
Proof. exact (0,, empty_fun). Defined.
...
(* a bit of luck: *)
Goal ∀ X, ∀ l l' : stn 0 -> X, ∀ x:X, append_fun l x = append_fun l' x.
  reflexivity.
Defined.

Definition compute_pr1_dni_last n i : pr1 (dni n (lastelement _) i) = pr1 i.
Proof.
  unfold dni,di; simpl. induction (natlthorgeh i n) as [q|q].
  - reflexivity.
  - contradicts (pr2 i) (natlehneggth q).
Defined.

Definition replace_dni_last n : dni n (lastelement _) = dni_lastelement.
Proof. apply funextfun; intros i. apply isinjstntonat. exact (compute_pr1_dni_last n i). Defined.
...
Proof.
  apply pair_path_in2. apply isapropifcontr. apply stn0_fun_iscontr.
Defined.

Definition isaset_transportf {X : hSet} (P : X -> UU) {x : X} (e : x = x) (p : P x) :
  transportf P e p = p.
(* move upstream *)
Proof. induction (pr1 ((setproperty _) _ _ (idpath _) e)). reflexivity. Defined.
...
Proof. intro. induction (pr1 (isapropifcontr i x0 x)). exact p0. Defined.
...
Proof. exact (invmap (weqsecovercontr P i) p0). Defined.
...
Proof. now apply isasetifcontr. Defined.

Definition iscontr_rect X (is : iscontr X) (x0 : X) (P : X -> UU) (p0 : P x0) : ∀ x:X, P x.
Proof. intro. exact (transportf P (pr1 (isapropifcontr is x0 x)) p0). Defined.

Definition iscontr_rect_compute X (is : iscontr X) (x : X) (P : X -> UU) (p : P x) :
  iscontr_rect X is x P p x = p.
Proof. unfold iscontr_rect. now rewrite iscontr_adjointness. Defined.
...
Proof.
  intros h. induction x as [n x]. induction n as [|n].
...
Proof. apply drop_and_append. Defined.
...
Proof.
  refine (weqgradth disassembleSequence assembleSequence _ _).
...
Goal ∀ X (x:Sequence X) (y:X), invmap SequenceAssembly (ii2 (y,,x)) = append x y.
  reflexivity.
Defined.
...
Definition Sequence_rect {X} {P : Sequence X ->UU}
           ...
           (x : Sequence X) : P x.
Proof.
  induction x as [n x]. induction n as [|n IH].
...
Proof.
  ...
  -
    ...
    intermediate_path (ind nil x p0). (* exploit the bit of luck we had above *)
    +
      admit.
    +
...
Proof.
  ...
  - intro i; simpl in i. simpl. now rewrite weqfromcoprodofstn_r0'.
Defined.
...
Proof.
  intros [[i x]|y]; reflexivity. (* amazingly easy, why? *)
Defined.
...
Proof. apply (total2_step (stn ∘ f)). Defined.
