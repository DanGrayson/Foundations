...
(** Comment: unfortunately [ weqsubquot ] is not as useful as it should be at moment due to the failure of the following code to work:
*)

Goal ∀ (X:UU) (R:eqrel X) (P:hsubtypes (setquot R)) (x:X) (is:P (setquotpr R x)),
  setquotpr (reseqrel R (funcomp (setquotpr R) P)) (tpair _ x is)
  = fromsubquot R P (tpair _ (setquotpr R x) is) .
Proof . intros . try apply idpath . try apply idpath . Abort.
